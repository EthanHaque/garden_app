...
const processJob = async (job) => {
    ...
    await job.updateProgress({ stage: "Starting", percentage: 5 });
    ...
    if (contentType.includes("application/pdf")) {
        ...
        // HTML Processing
        await job.updateProgress({ stage: "Extracting HTML", percentage: 30 });
        const htmlContent = await page.content();
        const extractedText = await page.evaluate(() => document.body.innerText);

        await job.updateProgress({ stage: "Chunking Text", percentage: 60 });
        const splitter = new RecursiveCharacterTextSplitter({ chunkSize: 1000, chunkOverlap: 100 });
        const chunks = await splitter.splitText(extractedText);

        await job.updateProgress({ stage: "Generating Embeddings", percentage: 80 });
